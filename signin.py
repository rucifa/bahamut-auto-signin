...
def fetch_csrf_token_from_api(baharune: str) -> str:
    """
    優先呼叫巴哈 CSRF API 取得合法 token
    失敗則備援使用 Cookie 中的 ckBahamutCsrfToken
    """
    urls_to_try = [
...
def fetch_answer_from_blackxblue() -> tuple[str, str]:
    ...
    for item in items:
        ...
        if answer:
            print(f"解析到答案：{answer}，API 直接取得，sn={csn}")
            return answer, f"API 直接取得，sn={csn}"
        ...
        if answer2:
            print(f"解析到答案：{answer2}，文章頁取得，sn={csn}")
            return answer2, f"文章頁取得，sn={csn}"

        print(f"[DEBUG] 文章頁內容前 800 字：{resp2.text[:800]}")
        raise Exception(f"找到文章（sn={csn}，標題：{title}）但無法解析答案，格式可能已變更")

    raise Exception(
        f"最新 3 篇文章中沒有今天（{today}）的，blackxblue 可能今天尚未發文"
    )
